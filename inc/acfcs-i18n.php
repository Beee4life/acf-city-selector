<?php
    // These are defined here so they are 'picked up' as translatable strings, because these values don't occur in the plugin itself.
    $andorra       = __( 'Andorra', 'acf-city-selector' );
    $austria       = __( 'Austria', 'acf-city-selector' );
    $australia     = __( 'Australia', 'acf-city-selector' );
    $brazil        = __( 'Brazil', 'acf-city-selector' );
    $canada        = __( 'Canada', 'acf-city-selector' );
    $europe        = __( 'Europe', 'acf-city-selector' );
    $france        = __( 'France', 'acf-city-selector' );
    $germany       = __( 'Germany', 'acf-city-selector' );
    $greatbritain  = __( 'Great Britain', 'acf-city-selector' );
    $luxembourg    = __( 'Luxembourg', 'acf-city-selector' );
    $mexico        = __( 'Mexico', 'acf-city-selector' );
    $portugal      = __( 'Portugal', 'acf-city-selector' );
    $spain         = __( 'Spain', 'acf-city-selector' );
    $switzerland   = __( 'Switzerland', 'acf-city-selector' );
    $united_states = __( 'United States', 'acf-city-selector' );
    $uruguay       = __( 'Uruguay', 'acf-city-selector' );
