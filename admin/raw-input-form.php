...
?>
<form method="post">
    <input name="acfcs_import_raw_nonce" type="hidden" value="<?php echo esc_attr( wp_create_nonce( 'acfcs-import-raw-nonce' ) ); ?>" />
    <?php echo sprintf( '<label for="raw-import">%s</label>', esc_attr__( 'Raw CSV import', 'acf-city-selector' ) ); ?>
    <?php echo sprintf( '<textarea name="acfcs_raw_csv_import" id="raw-import" rows="5" placeholder="%s">%s</textarea>', esc_attr( $placeholder ), esc_attr( $submitted_raw_data ) ); ?>
    <br />
    <?php
