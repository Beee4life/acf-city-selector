<?php
    ...
    $acfcs_selected_file_name = false;
    if ( isset( $_POST[ 'acfcs_select_file_nonce' ] ) ) {
        if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST[ 'acfcs_select_file_nonce' ] ) ), 'acfcs-select-file-nonce' ) ) {
            ...
            $acfcs_selected_file_name = isset( $_POST[ 'acfcs_file_name' ] ) ? sanitize_text_field( wp_unslash( $_POST[ 'acfcs_file_name' ] ) ) : false;
        }
    }
?>
<form method="post">
    ...
    <div class="acfcs__process-file">
        <div class="acfcs__process-file-element acfcs__process-file-element--file">
            ...
            <select name="acfcs_file_name" id="acfcs_file_name">
                ...
                <?php } ?>
                <?php foreach ( $file_index as $acfcs_file_name ) { ?>
                    <?php $acfcs_selected_file = $acfcs_selected_file_name == $acfcs_file_name ? ' selected="selected"' : false; ?>
                    <?php echo sprintf( '<option value="%s"%s>%s</option>', esc_attr( $acfcs_file_name ), esc_attr( $acfcs_selected_file ), esc_attr( $acfcs_file_name ) ); ?>
                <?php } ?>
            </select>
        </div>
        ...
        <div class="acfcs__process-file-element acfcs__process-file-element--delimiter">
            ...
            <select name="acfcs_delimiter" id="acfcs_delimiter">
                <?php foreach( $acfcs_delimiters as $acfcs_delimiter ) { ?>
                    <?php $acfcs_selected_delimiter = ( $acfcs_delimiter == apply_filters( 'acfcs_delimiter', ';' ) ) ? ' selected' : false; ?>
                    <?php echo sprintf( '<option value="%s"%s>%s</option>', esc_attr( $acfcs_delimiter ), esc_attr( $acfcs_selected_delimiter ), esc_attr( $acfcs_delimiter ) ); ?>
                <?php } ?>
            </select>
        </div>
